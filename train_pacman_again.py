...
DEFAULT_ENGINE_CONFIG = EngineConfig(
    ...
    height=584,
    quality_level= 4,
    time_scale= 1, #anything more than 11 and physics breaks
    target_frame_rate=-1,
...
def _unity_env_from_path_or_registry(env: str, **kwargs: Any) -> UnityEnvironment:
...
def train_ppo_unity_baseline(env_path: str,
    ...
    pretrained_model = PPO.load(PRE_TRAINED_PATH)
    ...
    new_model.learn(total_timesteps=timesteps, callback=[checkpoint_callback,eval_callback]) # eval needs to be fixed. environment may need to be wrapped in monitor

    # Save final model
    # model.save(os.path.join(model_save_path, "ppo_pacman"))
    new_model.save(os.path.join(model_save_path, "PPO_pacman_retrained"))
...
if __name__ == '__main__':
    ...
    config = {
        ...
        "pretrained_path":wandb_config["pretrained_path"],
    ...
    PRE_TRAINED_PATH = config["pretrained_path"]

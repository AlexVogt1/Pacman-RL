...
def get_schedule_fn(schedule_type: str, initial_lr: float, **kwargs) -> Callable[[float], float]:
    """
    Returns a function that takes `progress_remaining` (1.0 -> 0.0) and returns the current learning rate.
    """
    if schedule_type == "linear":
        return lambda progress: progress * initial_lr

    elif schedule_type == "rex":
        # Reflected Exponential: Convex decay
        def func(progress):
            progress_done = 1.0 - progress
            return initial_lr * (math.exp(1 - progress_done) - 1) / (math.e - 1)
        ...
        def func(progress):
            progress_done = 1.0 - progress
            steps_passed = int(progress_done / step_size)  # Use int() for cleaner steps
            return initial_lr * (gamma ** steps_passed)
        ...
        def func(progress):
            progress_done = 1.0 - progress
            return initial_lr * (decay_rate ** (progress_done * 10))
        ...
        def func(progress):
            progress_done = 1.0 - progress
            cosine_decay = 0.5 * (1 + math.cos(math.pi * progress_done))
            return min_lr + (initial_lr - min_lr) * cosine_decay

        return func

    elif schedule_type == "warmup":
        warmup_pct = kwargs.get("warmup_pct", 0.1)

        def func(progress):
            progress_done = 1.0 - progress
            if progress_done < warmup_pct:
                # Linear warmup
                return initial_lr * (progress_done / warmup_pct)
            else:
                # Linear decay for remaining progress
                remaining_progress = (1.0 - progress_done) / (1.0 - warmup_pct)
                return initial_lr * remaining_progress

        return func

    return lambda x: initial_lr  # Default constant
...
def train_ppo_unity_baseline(env_path: str,
    ...
    # create learning rate scheduler if possible
    try:
        learning_rate = get_schedule_fn(schedule_type=config['scheduler'], initial_lr=config['learning_rate'])
    except:
        print("no learning rate schedule provided just using constant learning rate from json")
